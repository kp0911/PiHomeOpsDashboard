...
여기서 `sda1`이 10TB HDD 파티션일 수 있습니다.
...
대상 디스크가 10TB 외장 HDD인지 반드시 확인하세요.
```
...
성공하면 `/mnt/nas`가 10TB 디스크로 보여야 합니다.
...
- [ ] SSH key 접속이 된다.
- [ ] 비밀번호 SSH 로그인을 껐다.
- [ ] 10TB HDD가 `/mnt/nas`에 마운트된다.
- [ ] 재부팅 후에도 `/mnt/nas`가 유지된다.
- [ ] Samba로 Windows에서 공유 폴더에 접근 가능하다.
- [ ] Docker가 설치되어 있다.
- [ ] `.env`의 기본 비밀번호를 바꿨다.
- [ ] `docker compose up -d --build`가 성공했다.
- [ ] `http://<Pi-Tailscale-IP>:8088`로 대시보드에 접속된다.
